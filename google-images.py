...
def next_link(s):
    ...
    if start_line == -1:    #If no links are found then give an error!
        ...
        content = str(s[start_content+6:end_content-1])
...
def search():
    ...
    search_keyword = input("Enter key words: ")
    
    #Download Image Links
    links = []
    search_keyword = search_keyword.replace(" ","%20")
    url = "https://www.google.com/search?as_st=y&tbm=isch&as_q=" + search_keyword + "&as_epq=&as_oq=&as_eq=&cr=&as_sitesearch=&safe=images&tbs=isz:lt,islt:svga,itp:photo,ift:jpg"
    html =  (get_html(url))
